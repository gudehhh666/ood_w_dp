...
parser.add_argument('--method', type=str, default='EL2N_cut_abl', help='method')
...
parser.add_argument('--mod', default='middle', choices=['highest', 'lowest', 'middle', 'random', 'two_side', 'uniform'],help='model directory')
parser.add_argument('--start', default=25, type=int, help='start epoch')
parser.add_argument('--end', default=175, type=int, help='end epoch')
parser.add_argument('--mid_cut', default=False, action='store_true', help='cut the middle')
...
if args.model_dir =='':
    model_dir = os.path.join(model_dir, f'{args.method}_r{args.rate}_cut', f'{args.start}_{args.end}_{args.mid_cut}')
else:
...
if args.dataset == 'cifar10':
    ...
    if args.method == 'infobatch':
        ...
        if args.method in ['EL2N', 'loss', 'EL2N_abl']:
            loss_dir = os.path.join(data_dir, args.model, args.method+'.pkl')
            print('loss_dir', loss_dir)
        else:
            loss_dir = None
        trainset = CIFAR10_loss_ablation(root=data_dir, train=True, download=True, transform=transform_train, loss_dir=loss_dir, cut_rate=args.rate, mod=args.mod)
    ...
    if args.method == 'infobatch':
        ...
        if args.method in ['EL2N', 'loss', 'EL2N_abl', 'EL2N_cut_abl']:
            loss_dir = os.path.join(data_dir, args.model, 'EL2N'+'.pkl')
            print('loss_dir', loss_dir)
        else:
            loss_dir = None
        trainset_cut = CIFAR100_loss_ablation(root=data_dir, train=True, download=True, transform=transform_train, loss_dir=loss_dir, cut_rate=args.rate, mod=args.mod)
        trainset_total = torchvision.datasets.CIFAR100(root=data_dir, train=True, download=True, transform=transform_test)

        train_loader_cut = torch.utils.data.DataLoader(trainset_cut, batch_size=args.batch_size, shuffle=True, **kwargs)
        train_loader_total = torch.utils.data.DataLoader(trainset_total, batch_size=args.batch_size, shuffle=True, **kwargs)
        testset = torchvision.datasets.CIFAR100(root=data_dir, train=False, download=True, transform=transform_test)
        test_loader = torch.utils.data.DataLoader(testset, batch_size=args.test_batch_size, shuffle=False, **kwargs)
...
#                                                     steps_per_epoch=len(train_loader_total),
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.mid_cut:
            if epoch >= args.start and epoch <= args.end:
                train_loader = train_loader_cut
            else:
                train_loader = train_loader_total
        else:
            if epoch >= args.start and epoch <= args.end:
                train_loader = train_loader_total
            else:
