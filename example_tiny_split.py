...
parser.add_argument('--dataset', '-d', default='tiny-imagenet', choices=dataset_options)
...
methods = ['baseline', 'infobatch', 'infobatchh_grad'] # 'infobatch'
parser.add_argument('--method', type=str, default='infobatch_grad', help='method')
parser.add_argument('--subset', type=bool, default=True)
parser.add_argument('--model-dir', default='/root/code_cc/checkpoint/test_split/inf_grad', help='directory of model for saving checkpoint')
# parser.add_argument()
...
print(f'==========>load the dataset with {args.dataset}')
# load the dataset
if args.dataset == 'cifar10':
    data_dir = '/lustre/home/xmwang/share/data/cifar10'    
    from data.cifar import get_cifar10
    train_loader, test_loader, trainset, testset = get_cifar10(data_dir, test_only=False, return_set=True, batch_size=args.batch_size, **kwargs)
    num_classes = 10
        
elif args.dataset == 'cifar100':
    
    data_dir = '/root/dataset/cifar100'
    from data.cifar import get_cifar100
    train_loader, test_loader, trainset, testset = get_cifar100(data_dir, test_only=False, return_set=True, batch_size=args.batch_size, **kwargs)
    num_classes = 100
elif args.dataset == 'cifar100-noisy':
    from data.cifar import get_cifar100, get_cifar100_mislabel
    data_dir = '/root/dataset/cifar100'
    test_loader = get_cifar100(data_dir, test_only=True, return_set=False, batch_size=args.batch_size, **kwargs)
    train_loader, trainset = get_cifar100_mislabel(data_dir, return_set=True, batch_size=args.batch_size, **kwargs)
    num_classes = 100
    ...
    train_loader, test_loader, trainset, testset = get_tiny(root, False, True, args.batch_size, **kwargs)

    
elif args.dataset == 'tiny-imagenet-noisy':
    num_classes = 200
    from data.Tiny_imagenet import get_tiny, get_tiny_mislabel
    root = {'train': '/root/tiny-imagenet/train',
            'test': '/root/tiny-imagenet/valid'}
    test_loader, testset = get_tiny(root, True, True, args.batch_size, **kwargs)
    train_loader, trainset = get_tiny_mislabel(root, return_set=True, batch_size=args.batch_size, **kwargs)
    

if args.subset:
    num_samples = len(trainset)
    subset_size = num_samples // 4
    remaining_samples = num_samples - subset_size * 3

    # 分割数据集
    subset_sizes = [subset_size] * 3 + [remaining_samples]
    print(subset_sizes)
...
#     print('==========>reload the dataset with infobatch')
...
def train(model, device, train_loader, trainset, optimizer):
    model.train()
    start_time = time.time()
    train_loss = 0
    correct = 0
...
def main():
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[40, 70], gamma=0.1)
    ...
    for subset in subsets:
        if args.method == 'infobatch' or args.method == 'infobatch_grad' or args.method =='infobatch_sparse':
            print('==========>reload the dataset with infobatch')
            subset = InfoBatch(subset, (args.epochs + 1)//(len(subsets)+1), args.rate, args.delta)
            train_loader = torch.utils.data.DataLoader(
                    subset, batch_size=args.batch_size, shuffle=args.shuffle, sampler = subset.sampler)
        for epoch in range(1, (args.epochs + 1)//(len(subsets)+1)):
            # temp_lr = adjust_learning_rate(optimizer, epoch)
            
            train_loss, train_acc, train_time = train(model, device, train_loader, subset, optimizer)
            
            # just eval on the test set
            test_loss, test_accuracy, test_time = eval_test(model, device, test_loader)
        
    for epoch in range(1,  25):
        trainset = InfoBatch(trainset, 25, args.rate, args.delta)
        train_loader = torch.utils.data.DataLoader(
                    trainset, batch_size=args.batch_size, shuffle=args.shuffle, sampler = trainset.sampler)
