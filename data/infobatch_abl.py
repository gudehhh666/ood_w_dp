...
class InfoBatch(Dataset):
    ...
    def __init__(self, dataset: Dataset, num_epochs: int,
                 prune_ratio: float = 0.5, delta: float = 0.875, save_high: float = 0.9):
        self.save_high = save_high
    ...
    def prune(self):
        ...
        # mean_value = self.scores.mean()
        # std_valure = self.scores.std()
        # well_learned_mask = ((self.scores > mean_value + std_valure) | (self.scores < mean_value - std_valure)).numpy()
        well_learned_mask = (self.scores < self.scores.mean()).numpy()
        # print(mean_value, std_valure)
        # print('well learned indices', well_learned_mask.shape)
        well_learned_indices = np.where(well_learned_mask)[0]
        remained_indices = np.where(~well_learned_mask)[0].tolist()
        print('remained indices', len(remained_indices))
        print('well learned indices', len(well_learned_indices))
        ...
        if len(selected_indices) > 0:
            remained_indices = np.random.choice(remained_indices, int(len(remained_indices) * self.save_high), replace=False).tolist()
            print('remained indices', len(remained_indices))

        self.reset_weights()
        if len(selected_indices) > 0:
            self.weights[remained_indices] = 1 /self.save_high
            self.weights[selected_indices] = 1 / self.keep_ratio
            remained_indices.extend(selected_indices)
        print('keep_ratio', self.keep_ratio)
        print('prune %d samples, #remained samples %d' % (len(self.dataset) - len(remained_indices), len(remained_indices)))
    ...
    @property
    def stop_prune(self):
        # print('stop prune at epoch %d' % (self.num_epochs * self.delta))
...
class IBSampler(object):
    ...
    def reset(self):
        ...
        if self.iterations > self.stop_prune//2:
            self.dataset.keep_ratio = 0.5
        if self.iterations > self.stop_prune:
            print('we are going to stop prune, #cur iterations %d, #stop prune %d' % (self.iterations, self.stop_prune))
            if self.iterations == self.stop_prune + 1:
                self.dataset.reset_weights()
            self.sample_indices = self.dataset.no_prune()
        else:
            print('we are going to continue pruning, #cur iterations %d, #stop prune %d' % (self.iterations, self.stop_prune))
