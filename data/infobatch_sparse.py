...
class IBSampler(object):
    ...
    def reset(self):
        ...
        if self.iterations > self.stop_prune:
            print('we are going to stop prune, #cur iterations stop prune %d, #stop prune %d' % (self.iterations, self.stop_prune))
            if self.iterations == self.stop_prune + 1:
                self.dataset.reset_weights()
            self.sample_indices = self.dataset.no_prune()
        elif self.iterations > 5 and self.iterations % 10 != 0:
            self.sample_indices = self.sample_indices
            print('we are going to keep the same pruning, #cur iterations stop prune %d, #stop prune %d' % (self.iterations, self.stop_prune))
        else:
            print('we are going to continue pruning, #cur iterations stop prune %d, #stop prune %d' % (self.iterations, self.stop_prune))
