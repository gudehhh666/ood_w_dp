...
class InfoBatch(Dataset):
    ...
    def __init__(self, dataset: Dataset, num_epochs: int,
        ...
        self.scores_old = torch.ones(len(self.dataset)) * 3
    ...
    def update(self, values):
        ...
        # print('infobatch values mean', values.mean())
        # print('infobatch weigth mean', weights.mean())
        values.mul_(weights)
        return values.mean()
    ...
    def prune(self):
        ...
        print("mean", self.scores.mean())
        print('old_mean', self.scores_old.mean())
        if self.scores_old.mean() == 3.0:
            print('scores are not updated in first step')
            grad = torch.zeros_like(self.scores)
        else:
            grad = torch.abs(self.scores_old - self.scores)
        well_learned_mask = (grad < grad.mean()).numpy()
        
        self.scores_old = self.scores.clone()

        print('mean grad', grad.mean())
        # print('mean after', self.scores.mean())
        # print('mean_old after', self.scores_old.mean())
...
class IBSampler(object):
    ...
    def reset(self):
        ...
        if self.iterations > self.stop_prune:
            print('we are going to stop prune, #cur iterationsstop prune %d, #stop prune %d' % (self.iterations, self.stop_prune))
            if self.iterations == self.stop_prune + 1:
                self.dataset.reset_weights()
            self.sample_indices = self.dataset.no_prune()
        else:
            print('we are going to continue pruning, #cur iterationsstop prune %d, #stop prune %d' % (self.iterations, self.stop_prune))
